...
pub const PROBLEM_URL_FILENAME: &str = ".url.txt";
...
impl KprRepository {
    ...
    pub fn find_repoconfig_path(current_dir: impl AsRef<Path>) -> Option<PathBuf> {
    ...
    pub fn from_fs(current_dir: impl AsRef<Path>) -> anyhow::Result<Self> {
        let Some(config_path) = Self::find_repoconfig_path(current_dir) else {
            bail!("Not in a kpr-repository dir: '{}' not found.", REPOSITORY_CONFIG_FILENAME);
        };
        let contents = storage::util::read_to_string(&config_path).context("Failed to read")?;
        let mut r = Self::from_toml(&contents)
            .with_context(|| format!("Invalid TOML: {}", config_path.to_string_lossy()))?;
        r = r.with_root_dir(config_path.parent().unwrap());
        Ok(r)
    }

    pub fn with_root_dir(mut self, root: impl AsRef<Path>) -> Self {
        let root = root.as_ref();
        self.root_dir = root.to_owned();
        self.vault_home = root.join(self.vault_home);
        self.daily_home = root.join(self.daily_home);
        self.solvespace_template = root.join(self.solvespace_template);
        self
    }
}
...
mod test {
    ...
    fn example_toml_should_be_parsable() {
        ...
        let r = KprRepository::from_toml(&toml).unwrap_or_else(|e| {
            panic!("{}", e);
        });
        assert_eq!(r.root_dir, Path::new(""));
